...
typedef struct SrcBufferPlayedCallback
{
	void(*callback)(void*);
	void *data;
} SrcBufferPlayedCallback;
...
ALvoid aluMixData(ALCdevice *device, ALvoid *buffer, ALsizei size)
{
    ...
    FPUCtl oldMode;
    ALuint i, c;
	ALuint oldBuffersPlayed;
	SrcBufferPlayedCallback callbacks[256];
	SrcBufferPlayedCallback *pCb, *pCbEnd;
    ...
    while(size > 0)
    {
        ...
        while(ctx)
        {
            ...
            while(src != src_end)
            {
				...
				oldBuffersPlayed = (*src)->BuffersPlayed;

				MixSource(*src, device, SamplesToDo);

				// if a buffer was played, add a callback
				if((*src)->BuffersPlayed > oldBuffersPlayed && (*src)->cbBufferPlayed != NULL)
				{
					// check if callback was already added
					for(pCb = callbacks; pCb != pCbEnd; ++pCb)
					{
						// callback is already in the list
						if(pCb->callback == (*src)->cbBufferPlayed)
							break;
					}

					if(pCb == pCbEnd)
					{
						pCbEnd->callback = (*src)->cbBufferPlayed;
						pCbEnd->data = (*src)->pCallbackParam;
						++pCbEnd;
					}
				}
